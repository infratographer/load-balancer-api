...
func testLocationsSoftDelete(t *testing.T) {
	t.Parallel()
...
func testLocationsQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Locations().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testLocationsSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testLocationsQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Locations().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testLocationsExists(t *testing.T) {
	...
	e, err := LocationExists(ctx, tx, o.LocationID)
	if err != nil {
...
func testLocationsFind(t *testing.T) {
	...
	locationFound, err := FindLocation(ctx, tx, o.LocationID)
	if err != nil {
		t.Error(err)
	}
...
func testLocationToManyLoadBalancers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, loadBalancerDBTypes, false, loadBalancerColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, loadBalancerDBTypes, false, loadBalancerColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}

	b.LocationID = a.LocationID
	c.LocationID = a.LocationID
	...
	check, err := a.LoadBalancers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadLoadBalancers(ctx, tx, false, (*[]*Location)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.LoadBalancers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.LoadBalancers = nil
	if err = a.L.LoadLoadBalancers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.LoadBalancers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testLocationToManyAddOpLoadBalancers(t *testing.T) {
	var err error
	...
	foreigners := []*LoadBalancer{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, loadBalancerDBTypes, false, strmangle.SetComplement(loadBalancerPrimaryKeyColumns, loadBalancerColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*LoadBalancer{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddLoadBalancers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.LocationID != first.LocationID {
			t.Error("foreign key was wrong value", a.LocationID, first.LocationID)
		}
		if a.LocationID != second.LocationID {
			t.Error("foreign key was wrong value", a.LocationID, second.LocationID)
		}
		...
		if a.R.LoadBalancers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.LoadBalancers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.LoadBalancers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	locationDBTypes = map[string]string{`CreatedAt`: `timestamptz`, `UpdatedAt`: `timestamptz`, `DeletedAt`: `timestamptz`, `LocationID`: `uuid`, `TenantID`: `uuid`, `DisplayName`: `string`}
	_               = bytes.MinRead
)
