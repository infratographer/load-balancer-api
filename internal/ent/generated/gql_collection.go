...
func (po *PortQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "pools":
			...
			args := newLoadBalancerPoolPaginateArgs(fieldArgs(ctx, new(LoadBalancerPoolWhereInput), path...))
			if err := validateFirstLast(args.first, args.last); err != nil {
				return fmt.Errorf("validate first and last in path %q: %w", path, err)
			}
			pager, err := newLoadBalancerPoolPager(args.opts, args.last != nil)
			if err != nil {
				return fmt.Errorf("create new pager in path %q: %w", path, err)
			}
			if query, err = pager.applyFilter(query); err != nil {
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					query := query.Clone()
					po.loadTotal = append(po.loadTotal, func(ctx context.Context, nodes []*Port) error {
						ids := make([]driver.Value, len(nodes))
						for i := range nodes {
							ids[i] = nodes[i].ID
						}
						var v []struct {
							NodeID gidx.PrefixedID `sql:"port_id"`
							Count  int             `sql:"count"`
						}
						query.Where(func(s *sql.Selector) {
							joinT := sql.Table(port.PoolsTable)
							s.Join(joinT).On(s.C(pool.FieldID), joinT.C(port.PoolsPrimaryKey[0]))
							s.Where(sql.InValues(joinT.C(port.PoolsPrimaryKey[1]), ids...))
							s.Select(joinT.C(port.PoolsPrimaryKey[1]), sql.Count("*"))
							s.GroupBy(joinT.C(port.PoolsPrimaryKey[1]))
						})
					...
					po.loadTotal = append(po.loadTotal, func(_ context.Context, nodes []*Port) error {
						for i := range nodes {
							n := len(nodes[i].Edges.Pools)
							if nodes[i].Edges.totalCount[0] == nil {
								nodes[i].Edges.totalCount[0] = make(map[string]int)
							}
							nodes[i].Edges.totalCount[0][alias] = n
			...
			path = append(path, edgesField, nodeField)
			if field := collectedField(ctx, path...); field != nil {
				if err := query.collectField(ctx, opCtx, *field, path, mayAddCondition(satisfies, "Pool")...); err != nil {
					return err
				}
			}
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				modify := limitRows(port.PoolsPrimaryKey[1], limit, pager.orderExpr(query))
				query.modifiers = append(query.modifiers, modify)
			} else {
				query = pager.applyOrder(query)
			}
